...
from pyfeat_classifier import PyFeatCognitiveClassifier


def main():
    print("[INFO] Inicializando sistema con Py-Feat...")
    print("[INFO] NOTA: Primera ejecución descargará modelos (~500MB)")
    
    detector = FaceDetector()
    classifier = PyFeatCognitiveClassifier()
    
    if not classifier.model_loaded:
        print("[ERROR] No se pudo cargar el modelo. Ejecuta:")
        print("  pip install -r requirements_pyfeat.txt")
        return
    ...
    while True:
        ...
        cv2.imshow('Analisis Cognitivo - Py-Feat', frame_display)
