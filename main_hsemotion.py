...
from hsemotion_classifier import HSEmotionCognitiveClassifier


def main():
    print("[INFO] Inicializando sistema con HSEmotion...")
    print("[INFO] NOTA: Primera ejecución descargará modelos")
    
    detector = FaceDetector()
    classifier = HSEmotionCognitiveClassifier()
    
    if not classifier.model_loaded:
        print("[ERROR] No se pudo cargar el modelo. Ejecuta:")
        print("  pip install -r requirements_hsemotion.txt")
        return
    ...
    while True:
        ...
        cv2.imshow('Analisis Cognitivo - HSEmotion', frame_display)
